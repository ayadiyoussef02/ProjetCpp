...
bool labo::ajouter(){
    ...
    query.prepare("insert into LABORATOIRES(ID_LABO,NOM_LABO,NTEL_LABO,SPECIALITE,ADRESSE,VIDEO,TEMP)""values(:ID_LABO,:NOM_LABO,:NTEL_LABO,:SPECIALITE,:ADRESSE,:VIDEO,:TEMP)");
...
QSqlQueryModel * labo::setnom()
 {

     QSqlQueryModel * model = new QSqlQueryModel();
     model->setQuery("select DISTINCT NOM_LABO from LABORATOIRES");
     model->setHeaderData(0,Qt::Horizontal,"NOM_LABO");

     return model;
 }
QSqlQueryModel *labo::sette(){
    QSqlQueryModel * model = new QSqlQueryModel();


        model->setQuery("SELECT NOM_LABO FROM LABORATOIRES where TEMP BETWEEN '0' AND '45' ");

        model->setHeaderData(0,Qt::Horizontal,QObject::tr("NOM_LABO"));
...
bool labo::modifiert(){
    QSqlQuery query;
    query.prepare("UPDATE LABORATOIRES SET  TEMP=:TEMP where ID_LABO=123 ");
